...
// Bắt sự kiện chấm công 
var intervalIdLoadLocation;
function getEventWindow(obj) {
  try {
    console.log("chay");
    var dotnet = obj;
    var timeReset = 0;
    ...
    function checkTimeReset() {
      timeReset += 1000;
      if (timeReset >= timeOut) {
        if (checkReset) {
          console.log("set onblur");
          dotnet.invokeMethodAsync("HandleOnBlur");
          checkReset = false
        }
      }
    }
    ...
    document.addEventListener("wheel", resetTime);
    intervalIdLoadLocation = setInterval(checkTimeReset, 1000);
  } catch (e) {
    console.log(e)
  }
}

function cancelIntervalLoadLocation() {
  if (intervalIdLoadLocation) {
    clearInterval(intervalIdLoadLocation);
  }
}

window.getCurrentPermission = async (obj) => {
  var allowPermission = null;
  console.log("asking permission");
  var dotNet = obj;
  navigator.geolocation.getCurrentPosition(function (position) {
    allowPermission = true;
    if (dotNet) dotNet.invokeMethodAsync("GetPermission", allowPermission);
  },
    function (error) {
...
function CustomDatepicker(obj, id) {
  ...
  var method = SetDropdownPosition(`.custom_datepicker.${id}`, ".custom_datepicker_calendar", obj);
...
function SetDropdownPosition(id, menuSelector, obj = null, method = null, scroll = null, checkClose) {
  if (currentDropdown) {
  ...
  if (currentDropdownOpen) {
    if (document.querySelector(".dropdown.is-active") || document.querySelector(".custom_datepicker_calendar.is-active")) {
  ...
  // bắt sự kiện scroll thì tắt datepicker
  if (currentDropdown.classList.contains("custom_datepicker")) {
    var elementScroll = document.querySelectorAll(".scrolly");
    if (elementScroll != null) {
      elementScroll.forEach((itemScroll) => {
        itemScroll.addEventListener('scroll', () => { obj.invokeMethodAsync("Close") })
      });
    }

    var elementScroll = document.querySelectorAll("#scrollbox");
    if (elementScroll != null) {
      elementScroll.forEach((itemScroll) => {
        itemScroll.addEventListener('scroll', () => { obj.invokeMethodAsync("Close") })
      });
  ...
  currentDropdown.top = currentDropdown.top <= 51 && !currentDropdown.classList.contains("custom_datepicker") ? currentDropdown.top = 52 : currentDropdown.top;
...
function startTimer(initialMinutes = 0, initialSeconds = 0, id) {
  if (document.getElementById(`timer${id}`) == null || document.getElementById(`timer${id}2`) == null)
    return 0;
  ...
  function updateTimer() {
    ...
    document.getElementById(`timer${id}2`).innerText = formattedTime;

    setLocalStorageSectionMeeting(`meeting_${id}`, formattedTime);
  }
...
function resetTimer(id) {
  var ele1 = document.getElementById(`timer${id}`);
  ...
  if (!ele1) {
    ...
    if (ele1)
      ele1.innerHTML = formattedTime;
  }
}

function setLocalStorageSectionMeeting(id, value) {
  localStorage.setItem(id, value);
}

function clearLocalStorageMeeting() {
  var removeList = [];

  for (var key in localStorage) {
    if (key.indexOf("meeting") != -1)
      removeList.push(key);
  }

  removeList.forEach((itemRemove) => {
...
function setLocalStorageOkr(value) {
  localStorage.setItem("_checkin_okrs", JSON.stringify(value));
}

window.handleBeforeUnload = function (event) {

  var currentPage = window.location.href;
  var targetPage = 'https://example.com/target-page'; // Replace with your target page URL

  // Check if the current page does not contain the specified pattern
  if (!currentPage.includes('/meeting/')) {
    return "leave";
  }
  return null;
};

function tableResize(id, dotnet) {
  var table = document.querySelector(".table_list");
  if (!table) return;

  var cols = table.querySelectorAll(".table_list_header li[data-code]");

  var selectedProperty;

  if (!cols) return;

  const firstPath = "fastdo_work";

  var tableHeight = table.offsetHeight;

  for (var i = 0; i < cols.length; i++) {
    //if (!cols[i].classList.contains("sticky")) {
    //  var div = createDiv(tableHeight);
    //  div.style.height = cols[i].offsetHeight + "px";
    //  cols[i].appendChild(div);
    //  cols[i].style.position = 'relative';
    //  cols[i].style.userSelect = "none";
    //  setListeners(div);
    //}
  ...
  setWidth(selectedProperty,dotnet);
  ...
  function setListeners(div) {
    ...
    div.addEventListener('mousedown', function (e) {
      ...
      selectedProperty = curCol.getAttribute("data-code");
    ...
    div.addEventListener('mouseover', function (e) {
      ...
      var child = e.currentTarget.firstChild;
      child.style.borderRight = '2px solid #0000ff30';
    })
  ...
  function createDiv(height) {
    ...
    div.style.background = '2px solid #ffffff20';

    insideDiv.style.top = 0;
    insideDiv.style.right = 0;
    insideDiv.style.width = '5px';
    insideDiv.style.position = 'absolute';
    insideDiv.style.cursor = 'col-resize';
    insideDiv.style.userSelect = 'none';
    insideDiv.style.pointerEvents = 'none';
    insideDiv.style.height = "100vh";
  ...
  function SendToLocalStorage() {
    ...
    var currentStorage = JSON.parse(localStorage.getItem("fastdo_work_list"));
    var array = [];
    if (current.includes("work")) {
      for (var i = 0; i < cols.length; i++) {
        var property = cols[i].getAttribute("data-code");
        var obj = {
          [`${property}`]: cols[i].offsetWidth
        }

        if (currentStorage) {
          currentStorage = Array.from(currentStorage).filter(x => !x[`${property}`]);
      ...
      if (currentStorage) {
        localStorage.setItem("fastdo_work_list", JSON.stringify(currentStorage));
      }
      else {
        localStorage.setItem("fastdo_work_list", JSON.stringify(array));
      }
      setWidth(selectedProperty, dotnet);
    }
  }
...
function setWidth(selectedProperty, dotnet) {
  if (dotnet == null) {
    var table = document.querySelector(".table_list");
    var cols = table.querySelectorAll(".table_list_header li");
    var defaultWidth = [];
    cols.forEach(x => defaultWidth.push(x.getAttribute("width")));

    var lastSizeData = JSON.parse(localStorage.getItem("fastdo_work_list"));
    var bodys = table.querySelectorAll(".list_view .task_list.is_item .column.is-narrow[data-code]");

    for (let i = 0; i < cols.length; i++) {
      var property = cols[i].getAttribute("data-code");
      if ((property == selectedProperty) || !selectedProperty) {
        var size = 0;
        if (lastSizeData) {
          var obj = Array.from(lastSizeData).find(x => x[`${property}`]);
          // Set Headers
          if (obj) {
            size = obj[`${property}`];
          }
          ...
          //if (!cols[i].classList.contains("sticky"))
          //  cols[i].style.width = size + "px";
          if (true)
            cols[i].style.width = size + "px";

          // Set Body
          var cells = Array.from(bodys).filter(x => x.getAttribute("data-code") == property);
          cells.forEach(x => {
            //if (!x.classList.contains("sticky"))
            //  x.style.width = size + "px";
            if (true)
              x.style.width = size - (property == "congviec" ? 32 : 0) + "px";
          });
        }
        else {
          // Set Headers
